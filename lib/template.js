...
module.exports = {
  ...
  HTML:function(title, list, body, control, userStatus='', state=''){
    ...
    <div class="jumbotron text-center">
        ...
        <ul class="tabs clearfix" data-tabgroup="first-tab-group">
          ...
          <li><a href="/movie?sort=title" target="_self" id="menu_genre">Movie by Genre</a></li>
  ...
  genre_list:function(genres, cate = '', sort = 'title'){
    var list = '<ul>';
    var i = 0;

    if(cate == ''){
      list = list + `<li style="display: inline-block; width: 100px;"><a href="/movie?sort=${sort}#genre_head" target="_self"><strong>전체</strong></a></li>`;
    }
    else{
      list = list + `<li style="display: inline-block; width: 100px;"><a href="/movie?sort=${sort}#genre_head" target="_self">전체</a></li>`;
    }
      ...
      if(cate == genres[i].G_ID){
        list = list + `
                      <li style="display: inline-block; width: 100px;">
                        <a href="/movie?category=${genres[i].G_ID}&sort=${sort}#genre_head"><strong>${sanitizeHtml(genres[i].G_Category)}</strong></a>
                      </li>
                      `;
      }
      else{
        list = list + `
                      <li style="display: inline-block; width: 100px;">
                        <a href="/movie?category=${genres[i].G_ID}&sort=${sort}#genre_head">${sanitizeHtml(genres[i].G_Category)}</a>
                      </li>
                      `;
      }
      if(i == 5){
    ...
    if(cate == ''){
      if(sort == 'title'){
        list = list + `
                      <div class="sorting">
                        <a href="/movie?sort=title#genre_head" target="_self"><strong>제목순</strong></a>
                        <a href="/movie?sort=rate#genre_head" target="_self" id="sort__">평점순</a>
                      </div>`;
      }
      else{
        list = list + `
                      <div class="sorting">
                        <a href="/movie?sort=title#genre_head" target="_self" id="sort__">제목순</a>
                        <a href="/movie?sort=rate#genre_head" target="_self"><strong>평점순</strong></a>
                      </div>`;
      }
    }
    else{
      if(sort == 'title'){
        list = list + `
                      <div class="sorting">
                      <a href="/movie?category=${cate}&sort=title#genre_head"><strong>제목순</strong></a>
                      <a href="/movie?category=${cate}&sort=rate#genre_head" id="sort__">평점순</a>
                      </div>`;
      }
      else{
        list = list + `
                      <div class="sorting">
                      <a href="/movie?category=${cate}&sort=title#genre_head" id="sort__">제목순</a>
                      <a href="/movie?category=${cate}&sort=rate#genre_head"><strong>평점순</strong></a>
                      </div>`;
      }
    }
  ...
  record_detail:function(record){
    ...
    body = body + `
                  ...
                  <p><br></p>
                  <div style="font-family: 'Jeju Gothic', serif;" class="container-fluid text-center">
                    <p>작성일   |  ${date}</p>
                    <p>평점     |  ${record[0].R_Grade}</p>
                    <p class="container" style="width: 500px;">한줄평   |  ${record[0].R_One_Eval}</p>
                    <p class="container" style="width: 500px;">감상문   |  ${record[0].R_Eval}</p>
                  </div>
                  <br>
                  `;
